...
async function revalidateAuth() {
	...
	if (validAuth["ok"]) {
		...
		revalidationTimeout = setTimeout(revalidateAuth, 60 * 1000 * 60);
		const nextCheck = new Date(Date.now() + 60 * 1000 * 60);
		console.log(`Revalidation set for ${nextCheck}`);
...
connectButton.addEventListener("click", async () => {
	...
	try {
		viewersInput.value = (await getViewerCount(userId, headers)) ?? "Error";
...
markButton.addEventListener("click", () => {
	const value = eventInput.value || "Event";
	events.push({ time: Date.now(), data: value });
